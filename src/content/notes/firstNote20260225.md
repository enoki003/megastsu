...
### 選定理由
要件定義内の制約にもある通り、このサイトは一人で制作し一人で管理する。そうなった場合、外部のCMSやMDを読み込む実装を組むのはコストが高い。
また、単純にMDを読み込むだけの実装だと、記事のメタデータ(frontmatter)の形式や必須項目が保証されず、長期的にコンテンツを運用する際に整合性が崩れるリスクがある。

これに対し Astro の Contents CollectionsではMDファイルのfrontmatterに対してスキーマを定義し、型レベルで検証が可能である。こうすることによって、形式の不備や必須項目の欠落を build 時に検出でき、完全性を高めることができる。
また、コンテンツを構造化されたデータとして扱うことが可能なため、記事一覧生成やフィルタリング処理も単純化できる。

以上の理由からAstroの標準機能であるContentCollectionを採用した。
